...
```{r boxregion, out.width = "100%", dpi = 300, echo=TRUE, warning=FALSE, fig.cap = "Boxplots for maximum length, width, thickness, stem length, and stem width for Perdiz arrow points recovered from the northern and southern behavioral regions."}
...
```{r pcaregion, out.width = "100%", dpi = 300, echo=TRUE, warning=FALSE, fig.cap = "Principal components analysis by north/south behavioral region."}
# attributes for plot
pch.gps.gp <- c(15,16)[as.factor(reg)]

## pca plot using correlation matrix
pca <- autoplot(prcomp(data[,4:8], scale = TRUE),
                data = data,
                asp = 1,
                shape = pch.gps.gp,
                colour = "region",
...
# perMANOVA
perdiz.matrix <- as.matrix(data[,4:8]) # response vars
perdiz.mat <- sqrt(perdiz.matrix) # sqrt transform
perdiz.dist <- vegdist(perdiz.mat,
                       method = 'bray') # dissimilarity matrix
set.seed(10) # make results reproducible

# model: perdiz morphology as a function of behavioural region
perdiz.ns <- adonis2(perdiz.dist ~ region,
                     data = data,
                     permutations = 10000,
                     method = 'bray')

## do point types differ by behavioural region?
perdiz.ns
```
